import monitor from "./js-tracker";
import { MonitorConfig, MonitorConfigArray } from "./types";
...
let commonConfig: MonitorConfig = _.clone(DEFAULT_CONFIG);
...
class Base {
    ...
    computeConfig(config = {}) {
        let _conf = this.isOverwrite
            ? { ...config }
            : _.merge(commonConfig, config);
        // 检测配置项
    ...
    // 各种监控初始化
    init(config: MonitorConfig = DEFAULT_CONFIG, isOverwrite = false) {
        // 是否为覆盖模式
        ...
        // 检查是否监控性能指标
        this.needRecordPerformance = _.get(
            commonConfig,
            ["record", "performance"],
            _.get(DEFAULT_CONFIG, ["record", "performance"])
        );
        // 检查是否监控JS错误
        this.needRecordJsError = _.get(
            commonConfig,
            ["record", "js_error"],
            _.get(DEFAULT_CONFIG, ["record", "js_error"])
        );
        // 检查是否监控用户在线时长
        this.needRecordTimeOnPage = _.get(
            commonConfig,
            ["record", "time_on_page"],
        ...
        monitor.init({
            concat: false,
            report: function (errorLogList: any[] = []) {
                // TODO 判断是否需要上报JS 错误
                ...
                for (let errorLog of errorLogList) {
                    ...
                    let errorName =
                        "页面报错_" + JS_TRACKER_ERROR_DISPLAY_MAP[type];
                    let location = window.location;
                    self.debugLogger(
                        "[自动]捕捉到页面错误, 发送打点数据, 上报内容 => ",
                        {
                            error_no: errorName,
    ...
    send(info = {}) {
        ...
        try {
            ...
            this.debugLogger(
                `config.getPageType执行时发生异常, 请注意, 错误信息=>`,
                {
                    e,
                    location,
                }
     ...
     * @param {展示数据} extra
     */
    log(type = "", code: number, detail = {}, extra = {}) {
    ...
    error(code: number, detail: any, extra?: any) {
        return this.log("error", code, detail, extra);
    }

    product(code: number, detail: any, extra?: any) {
        return this.log("product", code, detail, extra);
    }

    info(code: number, detail: any, extra?: any) {
        return this.log("info", code, detail, extra);
    }

    debugLogger(...args: any[]) {
        // 只有在测试时才打印log
        if (this.isTest) console.log(...args);
    }
}
...
class Logger extends Base {
    ...
    notify(errorName = "", url = "", extraInfo: any = {}) {
        // 规范请求参数
        let detail: any = {};
        let extra: any = {};
        if (!errorName) {
